    ...
    var body: some View {
		Button("\(progressBar.isAdded ? "Stop" : "Start") Download") {
			if progressBar.isAdded {
				...
				let config = ProgressConfig(title: "SMR", progressImage: "arrow.up", expandedImage: "clock.badge.checkmark.fill", tint: .yellow, rotationEnabled: true)
...
struct DynamicProgressView: View {
			...
			ctx.drawLayer { context in
				for index in [1,2] {
					if let resolvedImage = ctx.resolveSymbol(id: index) {
						context.draw(resolvedImage, at: .init(x: size.width / 2, y: 11 + 18))
					}
				}
			}
		} symbols: {
	...
	func CustomAlertView() -> some View {
		GeometryReader { proxy in
			...
			Capsule()
				.fill(.black)
				.frame(width: showAlertView ? size.width : 126, height: showAlertView
					   ? size.height : 37)
				.overlay(content: { 
					HStack(spacing: 13) {
